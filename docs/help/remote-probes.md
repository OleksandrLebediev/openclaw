...
- **Session isolation (persona-probe):** each case runs `openclaw agent` with a new `--session-id` (`<probeCaseId>-<uuid>`, explicit session), reads that turn’s transcript at `~/.openclaw/agents/<id>/sessions/<sessionId>.jsonl`, then best-effort deletes the session row from `sessions.json`, so cases do not share transcript history (see `scripts/dev/persona-probe/capture.ts`).
...
- **Filters:** `--group persona|traps|memory|tools`, `--tags a,b` (comma-separated), `--case id,id` (run only listed probe case ids).
- **Env defaults:** `PERSONA_PROBE_AGENT` (falls back to agent id `**lilu`\*\* if you omit `--agent` and the env var), `PERSONA_PROBE_HOST` (SSH alias; script default is `claw` when unset).
...
- **Env defaults:** `AVAILABILITY_PROBE_AGENT` / `AVAILABILITY_PROBE_HOST`, or reuse `**PERSONA_PROBE_AGENT`** / `**PERSONA_PROBE_HOST**`if the availability vars are unset; agent id falls back to`**lilu\*\*`when all agent env vars are unset and`--agent` is omitted.
