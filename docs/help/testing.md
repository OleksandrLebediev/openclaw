...
Optional **developer scripts** (not Vitest, not CI): they SSH to a gateway host, run **`openclaw agent --json`** once per case, and write reports under `.artifacts/*-probe/`. See [Remote probes (SSH)](/help/remote-probes) for **persona-probe** and **availability-probe**, prerequisites, filters, and how they differ from the suites in this doc.
