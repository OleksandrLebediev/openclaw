...
read_when:
  ...
  - Running persona-probe or availability-probe against a remote SSH host
title: "Testing"
---
...
## Remote SSH probes (dev)

These are **optional developer harnesses** under `scripts/dev/`. They are **not** part of `pnpm test` or CI. They SSH to a machine where the gateway (or CLI) is installed, run **`openclaw agent --json`** once per case, then score the outcome.
...
- **Entry:** `scripts/dev/availability-probe.ts`. **Cases:** `scripts/dev/availability-probe/cases.ts`.
- **What it tests:** Records **wall-clock time** for the full remote `openclaw agent` round trip (`durationMs`) and compares it to the same **`computeReadingDelayMs`** helper the gateway uses (`src/agents/availability.ts`). In other words: **did configured reading-speed delay show up at all** in end-to-end latency, given that the model still dominates timing.
- **Important caveats:**
  ...
  - Any case that sets **`derivedFromReadingSpeed` must match** `agents.defaults.availability.readingSpeed` / `agents.list[].availability.readingSpeed` on the **remote** host. If the numbers differ, the probe compares apples to oranges.
...
- **Filters:** `--group reading|sanity|offline`, `--tags` (comma-separated).
