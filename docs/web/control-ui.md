...
- Agents: per-agent workspace files, tools, skills, channel context, cron jobs, model selection; **Availability** tab edits optional `agents.list[].availability` (and shows defaults from `agents.defaults.availability`) for timezone (IANA dropdown plus `local`), active hours, busy windows, offline queue vs immediate replies, reading-speed delay, optional extra random delay after reading, and writing-speed delay — see [Configuration reference](/gateway/configuration-reference#agentsdefaultsavailability)
